...
{
    public static class RngGuesser8
    {
        public static void Guess()
        {
            ...
            var posGChars = Constants.RandomizeString(Constants.Chars);
            var posHChars = Constants.RandomizeString(Constants.Chars);
                            ...
                            for (int e = 0; e < availableCharsCount; e++)
                            {
                                for (int f = 0; f < availableCharsCount; f++)
                                {
                                    for (int g = 0; g < availableCharsCount; g++)
                                    {
                                        ...
                                        {
                                            if (posAChars[a] == pwd[0] && posBChars[b] == pwd[1] && posCChars[c] == pwd[2] && posDChars[d] == pwd[3] && posEChars[e] == pwd[4] && posFChars[f] == pwd[5] && posEChars[g] == pwd[6] && posFChars[h] == pwd[7])
                                            {
                                                sw.Stop();
